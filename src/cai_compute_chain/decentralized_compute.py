...
def _preflight_cai_owned_transport_node_route(
    ...
    return {
        ...
        "selectedRouteClass": _cai_owned_transport_peer_url_route_class(selected_url),
        "peerCaiUrls": urls,
    }
...
def _summary_resource_headroom_audit(
    summary: Mapping[str, Any],
    ...
    *,
    ...
    resources = _summary_resource_payload(summary, worker)
    ram_available = _summary_resource_bytes(
        resources,
        "ramAvailableBytes",
        "availableRamBytes",
        "ram_available_bytes",
    ...
    ram_total = _summary_resource_bytes(
        resources,
        "ramBytes",
        "ramTotalBytes",
        "ram_total_bytes",
    ...
    vram_available = _summary_resource_bytes(
        resources,
        "vramAvailableBytes",
        "availableVramBytes",
    ...
    vram_total = _summary_resource_bytes(
        resources,
        "vramBytes",
        "vramTotalBytes",
        "vram_total_bytes",
...
def _summary_available_ranges_from_candidate(
    candidate: Mapping[str, Any],
) -> list[dict[str, int]]:
    ranges: list[dict[str, int]] = []
    ranges.extend(_summary_layer_ranges([candidate], assume_ready=False))
    for field_name in (
        "shards",
        "layerRanges",
        "layer_ranges",
        ...
        if isinstance(value, Sequence) and not isinstance(value, (str, bytes)):
            ranges.extend(_summary_layer_ranges(value, assume_ready=False))
    return _summary_merge_layer_ranges(ranges)


def _summary_layer_ranges(
    values: Sequence[Any],
    *,
    assume_ready: bool,
) -> list[dict[str, int]]:
    ...
    for item in values:
        if not isinstance(item, Mapping):
            continue
        if not assume_ready and not _summary_layer_range_item_ready(item):
            continue
        start = _optional_int(
            item.get("layerStart")
...
def _summary_layer_range_item_ready(item: Mapping[str, Any]) -> bool:
    if _summary_layer_range_item_block_reason(item):
        return False
    ready = _summary_bool(
        item,
        "ready",
    ...
    if status in {"cached", "downloaded", "available"}:
        return _summary_cached_shard_integrity_satisfied(item)
    return _summary_can_load_before_deadline(item)


def _summary_blocked_ranges_from_candidate(
    candidate: Mapping[str, Any],
) -> list[dict[str, Any]]:
    blocked: list[dict[str, Any]] = []
    for item in _summary_candidate_range_items(candidate):
        if not isinstance(item, Mapping):
            continue
        reason = _summary_layer_range_item_block_reason(item)
        if not reason:
            continue
        ranges = _summary_layer_ranges([item], assume_ready=True)
        for layer_range in ranges:
            blocked.append(
                {
                    **layer_range,
                    "reason": reason,
                    "error": _summary_layer_range_item_block_error(reason),
                }
            )
    return blocked


def _summary_candidate_range_items(candidate: Mapping[str, Any]) -> list[Mapping[str, Any]]:
...
def _summary_layer_range_item_block_reason(item: Mapping[str, Any]) -> str | None:
    for field_name, reason in (
        ...
        if _summary_bool(item, field_name) is False:
            return reason

    encrypted = _summary_bool(
        item,
        "encryptedAtRest",
        "encrypted_at_rest",
        "encryptedCache",
    ...
    if encrypted is True and not _summary_encrypted_cache_accessible(item):
        return "encrypted_cache_key_missing"

    if _summary_deadline_expired(item) and not _summary_item_already_materialized(item):
        return "download_deadline_expired"

    return None


def _summary_layer_range_item_block_error(reason: str) -> str:
    errors = {
...
def _summary_cached_shard_integrity_satisfied(item: Mapping[str, Any]) -> bool:
    return _summary_layer_range_item_block_reason(item) is None


def _summary_encrypted_cache_accessible(item: Mapping[str, Any]) -> bool:
    if _summary_item_already_materialized(item):
        return True
    accessible = _summary_bool(
        item,
...
def _summary_item_already_materialized(item: Mapping[str, Any]) -> bool:
    materialized = _summary_bool(
        item,
        "loaded",
        "materialized",
        "decrypted",
...
def _summary_can_load_before_deadline(item: Mapping[str, Any]) -> bool:
    can_load = _summary_bool(
        item,
    ...
    return can_load is True and not _summary_deadline_expired(item)


def _summary_deadline_expired(item: Mapping[str, Any]) -> bool:
    for field_name in (
        "downloadDeadlineAt",
...
def _summary_merge_layer_ranges(
...
def _summary_missing_required_ranges(
...
def cai_owned_transport_version_compatibility(
    ...
    *,
    ...
    compatible_protocol_versions = _cai_owned_transport_int_list(
...
def _cai_owned_transport_version_label(value: object) -> str | None:
...
def _cai_owned_transport_int_list(value: object) -> list[int]:
    if value is None:
        return []
    ...
    for item in value:
        ...
        if integer is not None and integer not in cleaned:
...
def _normalize_cai_owned_transport_shard_ranges(
    executor_node_ids: Sequence[str],
    total_layer_count: int,
    *,
    shard_ranges: Sequence[Mapping[str, Any]] | None = None,
    ...
    if shard_ranges is None:
        return _cai_owned_transport_layer_ranges(executors, total_layers)
...
def _decode_cai_owned_transport_batch_payload(envelope: dict[str, Any]) -> bytes:
    encoded_payload_bytes = _encoded_cai_owned_transport_payload_bytes_from_envelope(
        envelope,
    )
    compression = str(envelope.get("payloadCompression") or "").strip().lower()
    ...
    return _decode_cai_owned_transport_payload_bytes(
        encoded_payload_bytes,
        compression=compression or None,
    )


def _encoded_cai_owned_transport_payload_bytes_from_envelope(
    envelope: dict[str, Any],
) -> bytes:
...
def _cai_owned_transport_encoded_payload_fields(
...
def _normalize_cai_owned_transport_payload_compression(
    value: str | None,
) -> str | None:
...
def _encode_cai_owned_transport_payload_bytes(
    payload: bytes,
    *,
...
def _decode_cai_owned_transport_payload_bytes(
    payload: bytes,
    *,
    compression: str | None,
) -> bytes:
...
def _runtime_metadata_text(metadata: Mapping[str, Any], *keys: str) -> str | None:
    for key in keys:
...
def _runtime_metadata_int(metadata: Mapping[str, Any], *keys: str) -> int | None:
    for key in keys:
        raw = metadata.get(key)
...
def _runtime_metadata_bool(metadata: Mapping[str, Any], *keys: str) -> bool | None:
    for key in keys:
        raw = metadata.get(key)
...
def _runtime_metadata_shape(metadata: Mapping[str, Any]) -> list[int] | None:
    raw = None
    for key in (
        "activationShape",
        "activation_shape",
...
def _require_runtime_metadata_layer_range_supported(
    runtime_metadata: Mapping[str, Any],
    *,
    model_id: str,
) -> None:
    shard_compatibility = _runtime_metadata_text(
        runtime_metadata,
        "shardCompatibility",
        "shard_compatibility",
    )
    layer_range_supported = _runtime_metadata_bool(
        runtime_metadata,
        "layerRangeSupported",
    ...
    if shard_compatibility and shard_compatibility != GGUF_SHARD_MODE_LAYER_RANGE_SUPPORTED:
        raise ValueError(
    ...
    gguf_architecture = _runtime_metadata_text(
        runtime_metadata,
        "ggufArchitecture",
        "gguf_architecture",
    )
    if gguf_architecture:
        compatibility = gguf_shard_compatibility(
            ...
            family=_runtime_metadata_text(runtime_metadata, "family"),
            filename=_runtime_metadata_text(
                runtime_metadata,
                "preferredFilename",
                "preferred_filename",
            ),
...
def _runtime_metadata_external_shard_descriptor(
    runtime_metadata: Mapping[str, Any],
) -> dict[str, Any]:
    output: dict[str, Any] = {}
    for key in (
        ...
        value = _runtime_metadata_text(runtime_metadata, key)
        if value:
            output[key] = value
    layer_range_supported = _runtime_metadata_bool(
        runtime_metadata,
    ...
    context_length = _runtime_metadata_int(
        runtime_metadata,
        "contextLength",
        "context_length",
        "nCtx",
