...
Thank you for your interest in contributing to cai!

## Getting Started

To run cai from source:
...
```bash
git clone https://github.com/CAI-explore/cai.git
cd CAI/dashboard
...
cai is built with a mix of Rust, Python, and TypeScript (Svelte for the dashboard), and the codebase is actively evolving. Before starting work:
...
cai uses TOML-based model cards to define model metadata and capabilities. Model cards are stored in:
- `resources/inference_model_cards/` for text generation models
- `resources/image_model_cards/` for image generation models
- `~/.CAI/custom_model_cards/` for user-added custom models
...
cai supports multiple API formats through an adapter pattern. Adapters convert API-specific request formats to the internal `TextGenerationTaskParams` format and convert internal token chunks back to API-specific responses.

### Adapter Architecture

All adapters live in `src/CAI/master/adapters/` and follow the same pattern:
...
1. Create a new adapter file in `src/CAI/master/adapters/`
...
4. Implement non-streaming response collection:
   ...
   async def collect_your_api_response(
       ...
       pass
   ```
5. Register the adapter endpoints in `src/CAI/master/api.py`
