...
<script lang="ts">
  ...
  async function openInPerfetto() {
    ...
    if (!perfettoWindow) {
      alert(tr("Failed to open Perfetto. Please allow popups."));
      return;
    }
  ...
  onMount(async () => {
    ...
    try {
      ...
      error = e instanceof Error ? e.message : tr("Failed to load trace");
    } finally {
      loading = false;
    }
  });
  ...
  <div class="max-w-7xl mx-auto px-4 lg:px-8 py-6 space-y-6">
    <div class="flex items-center justify-between gap-4 flex-wrap">
      <div>
        ...
        <p class="text-sm text-cai-light-gray font-mono truncate max-w-lg">
          {taskId}
        </p>
      </div>
      <div class="flex items-center gap-3">
        <a
          href="#/traces"
          class="text-xs font-mono text-cai-light-gray hover:text-cai-yellow transition-colors uppercase border border-cai-medium-gray/40 px-3 py-1.5 rounded"
        >
          {tr("All Traces")}
        </a>
        <button
        ...
        <button
          type="button"
          class="text-xs font-mono text-cai-dark-gray bg-cai-yellow hover:bg-cai-yellow/90 transition-colors uppercase px-3 py-1.5 rounded font-semibold"
    ...
    {#if loading}
      <div
        class="rounded border border-cai-medium-gray/30 bg-cai-black/30 p-6 text-center text-cai-light-gray"
      >
        <div class="text-sm">{tr("Loading trace data...")}</div>
      </div>
    {:else if error}
      <div
    ...
    {:else if stats}
      <!-- Wall Time Summary -->
      <div
        class="rounded border border-cai-medium-gray/30 bg-cai-black/30 p-4 space-y-2"
      >
        <h2
          class="text-sm font-mono uppercase tracking-wider text-cai-light-gray"
        >
          {tr("Summary")}
        </h2>
        <div class="text-3xl font-mono text-cai-yellow">
          {formatDuration(stats.totalWallTimeUs)}
        </div>
        <div class="text-xs text-cai-light-gray">{tr("Total wall time")}</div>
      </div>

      <!-- By Phase -->
      {#if phases.length > 0}
        <div
          class="rounded border border-cai-medium-gray/30 bg-cai-black/30 p-4 space-y-4"
        >
          <h2
            class="text-sm font-mono uppercase tracking-wider text-cai-light-gray"
          >
            {tr("By Phase")}
            <span class="text-cai-light-gray/50">{tr("(avg per node)")}</span>
          </h2>
          <div class="space-y-4">
              ...
              <div class="space-y-2">
                <div class="flex items-center justify-between">
                  ...
                  <span class="text-sm font-mono">
                    <span class="text-cai-yellow"
                      >{formatDuration(normalizedTotal)}</span
                    >
                    <span class="text-cai-light-gray ml-2">
                      {trf("({count} steps, {duration}/step)", {
                ...
                {#if phase.subcategories.length > 0}
                  <div class="pl-4 space-y-1.5">
                    {#each phase.subcategories as subcat}
                      ...
                      >
                        <span class="text-cai-light-gray">{subcat.name}</span>
                        <span class="text-white">
                          {formatDuration(normalizedSubcat)}
                          <span class="text-cai-light-gray ml-2">({pct})</span>
                          <span class="text-cai-light-gray/60 ml-2"
                            >{trf("{duration}/step", {
                              duration: formatDuration(perStep),
                            })}</span
                          >
                        </span>
                      </div>
                      ...
                      >
                        <div
                          class="absolute inset-y-0 left-0 bg-gradient-to-r from-cai-yellow to-cai-yellow/70 transition-all duration-300"
                          style="width: {pct}"
                        ></div>
                      </div>
                    {/each}
        ...
        >
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {#each sortedRanks as rank}
              ...
              <div
                class="rounded border border-cai-medium-gray/20 bg-cai-dark-gray/60 p-3 space-y-3"
              >
                <div class="text-sm font-mono text-cai-yellow">
                  {trf("Rank {rank}", { rank })}
                </div>
                <div class="space-y-2">
                  {#each rankPhases as phase}
                    <div class="space-y-1">
                      <div class="flex items-center justify-between text-xs">
                        <span class="font-mono text-cai-light-gray"
                          >{phase.name}</span
                        >
                        <span class="font-mono text-white">
                          {formatDuration(phase.totalUs)}
                          <span class="text-cai-light-gray/50 ml-1">
                            ({phase.stepCount}x)
                          </span>
                        </span>
                      </div>
                      {#if phase.subcategories.length > 0}
                        <div class="pl-2 space-y-0.5">
                          {#each phase.subcategories as subcat}
                            ...
                            >
                              <span class="text-cai-light-gray/70"
                                >{subcat.name}</span
                              >
                              <span class="text-cai-light-gray">
                                {formatDuration(subcat.stats.totalUs)}
                                <span class="text-cai-light-gray/50"
                                  >({pct})</span
                                >
                                <span class="text-cai-light-gray/30 ml-1"
                                  >{trf("{duration}/step", {
                                    duration: formatDuration(perStep),
                                  })}</span
                                >
