...
<script lang="ts">
  ...
  import { currentLocale, tr, trf } from "$lib/stores/i18n.svelte";
...
{#snippet deleteButton(nodeId: string, modelId: string)}
  <button
    ...
    title={tr("Delete from this node")}
  >
    {@render trashIcon()}
  </button>
{/snippet}

<div class="min-h-screen bg-cai-dark-gray text-white">
  <HeaderNav showHome={true} />
  <div class="max-w-7xl mx-auto px-4 lg:px-8 py-6 space-y-6">
    <div class="flex items-center justify-between gap-4 flex-wrap">
      <div>
        <h1
          class="text-2xl font-mono tracking-[0.2em] uppercase text-cai-yellow"
        >
          {tr("Downloads")}
        </h1>
        <p class="text-sm text-cai-light-gray">
          {tr("Overview of models on each node")}
        </p>
      </div>
      <div class="flex items-center gap-3">
        <button
          type="button"
          class="text-xs font-mono text-cai-light-gray hover:text-cai-yellow transition-colors uppercase border border-cai-medium-gray/40 px-2 py-1 rounded"
          onclick={() => refreshState()}
          title={tr("Force refresh from /state")}
        >
          {tr("Refresh")}
        </button>
        <div class="text-[11px] font-mono text-cai-light-gray">
          {tr("Last update:")} {lastUpdateTs
            ? new Date(lastUpdateTs).toLocaleTimeString(currentLocale())
            : tr("n/a")}
        </div>
      </div>
    </div>
    ...
    {#if !hasDownloads}
      <div
        class="rounded border border-cai-medium-gray/30 bg-cai-black/30 p-6 text-center text-cai-light-gray space-y-2"
      >
        <div class="text-sm">
          {tr("No downloads found. Start a model download to see progress here.")}
        </div>
        <div class="text-[11px] text-cai-light-gray/70">
          {tr("Download keys detected:")} {downloadKeys.length === 0
            ? tr("none")
            : downloadKeys.join(", ")}
        </div>
      </div>
    {:else}
      <div
        class="rounded border border-cai-medium-gray/30 bg-cai-black/30 overflow-x-auto"
      >
        <table class="w-full text-left font-mono text-xs">
          <thead>
            <tr class="border-b border-cai-medium-gray/30">
              <th
                class="sticky left-0 z-10 bg-cai-black px-4 py-3 text-[11px] uppercase tracking-wider text-cai-yellow font-medium whitespace-nowrap border-r border-cai-medium-gray/20"
              >
                {tr("Model")}
              </th>
              {#each nodeColumns as col}
                <th
                  class="px-4 py-3 text-[11px] uppercase tracking-wider text-cai-light-gray font-medium text-center whitespace-nowrap min-w-[120px]"
                >
                  <div>{col.label}</div>
                  {#if col.diskAvailable != null}
                    ...
                    >
                      {trf("{size} free", {
                        size: formatBytes(col.diskAvailable),
                      })}
                    </div>
                  {/if}
                </th>
          ...
          <tbody>
            {#each modelRows as row}
              <tr
                class="group border-b border-cai-medium-gray/20 hover:bg-cai-medium-gray/10 transition-colors"
              >
                <td
                  class="sticky left-0 z-10 bg-cai-dark-gray group-hover:bg-[oklch(0.18_0_0)] transition-colors px-4 py-3 whitespace-nowrap border-r border-cai-medium-gray/20"
                >
                  ...
                  <td class="px-4 py-3 text-center align-middle">
                    {#if cell.kind === "completed"}
                      <div
                        ...
                        title={trf("Completed ({size})", {
                          size: formatBytes(cell.totalBytes),
                        })}
                      >
                        <svg
                    ...
                    {:else if cell.kind === "downloading"}
                      <div
                        ...
                        title={`${formatBytes(
                          ...
                          cell.speed,
                        )} - ${tr("ETA")} ${formatEta(cell.etaMs)}`}
                      >
                        <span class="text-cai-yellow text-sm font-medium"
                          >{clampPercent(cell.percentage).toFixed(1)}%</span
                        >
                        <div
                          class="w-16 h-2 bg-cai-black/60 rounded-sm overflow-hidden"
                        >
                          <div
                            class="h-full bg-gradient-to-r from-cai-yellow to-cai-yellow/70 transition-all duration-300"
                        ...
                        <div class="flex gap-1 mt-0.5">
                          <button
                            type="button"
                            class="text-white/50 hover:text-cai-yellow transition-colors cursor-pointer"
                            onclick={() =>
                              cancelDownload(col.nodeId, row.modelId)}
                            title={tr("Pause download")}
                          >
                            {@render pauseIcon()}
                          </button>
                          {@render deleteButton(col.nodeId, row.modelId)}
                    ...
                    {:else if cell.kind === "pending"}
                      <div
                        ...
                        title={cell.downloaded > 0
                          ? trf("{downloaded} / {total} downloaded (paused)", {
                              downloaded: formatBytes(cell.downloaded),
                              total: formatBytes(cell.total),
                            })
                          : tr("Download pending")}
                      >
                        {#if cell.downloaded > 0 && cell.total > 0}
                          ...
                          >
                            <div
                              class="h-full bg-cai-light-gray/40 rounded-full"
                              style="width: {(
                          ...
                          <div class="flex gap-1">
                            {#if row.shardMetadata}
                              <button
                                ...
                                title={tr("Resume download on this node")}
                              >
                                {@render downloadIcon()}
                              </button>
                            {:else}
                        ...
                        {:else if row.shardMetadata}
                          <button
                            ...
                            onclick={() =>
                              startDownload(col.nodeId, row.shardMetadata!)}
                            title={tr("Start download on this node")}
                          >
                        ...
                        <div class="flex gap-1">
                          {#if row.shardMetadata}
                            <button
                              ...
                              onclick={() =>
                                startDownload(col.nodeId, row.shardMetadata!)}
                              title={tr("Retry download on this node")}
                            >
                              {@render downloadIcon()}
                            </button>
                          {/if}
                    ...
                    {:else}
                      <div
                        ...
                        title={tr("Not on this node")}
                      >
                        <span class="text-cai-medium-gray text-lg leading-none"
                          >--</span
                        >
                        {#if row.shardMetadata}
                          <button
                            type="button"
                            class="text-white/50 hover:text-cai-yellow transition-colors mt-0.5 opacity-0 group-hover:opacity-100 cursor-pointer"
                            onclick={() =>
                              startDownload(col.nodeId, row.shardMetadata!)}
                            title={tr("Download to this node")}
                          >
                            {@render downloadIcon()}
                          </button>
                        {/if}
...
{#if infoRow}
  ...
  >
    ...
    <div class="space-y-2 text-xs font-mono">
      <div class="flex items-center gap-2">
        <span class="text-white/40">{tr("Model ID:")}</span>
      ...
      {#if infoRow.modelCard}
        ...
        {#if infoRow.modelCard.nLayers > 0}
          <div class="flex items-center gap-2">
            <span class="text-white/40">{tr("Layers:")}</span>
        ...
        {/if}
        <div class="flex items-center gap-2">
          <span class="text-white/40">{tr("Tensor parallelism:")}</span>
          <span class="text-white/70"
            >{infoRow.modelCard.supportsTensor ? tr("Yes") : tr("No")}</span
          >
        </div>
      {/if}
      ...
      {#if nodeColumns.filter((col) => (infoRow?.cells[col.nodeId]?.kind ?? "not_present") !== "not_present").length > 0}
        <div class="mt-3 pt-3 border-t border-cai-yellow/10">
          <div class="flex items-center gap-2 mb-1">
            ...
            <span class="text-white/40">{tr("On nodes:")}</span>
          </div>
                  ...
                  <span
                    ...
                    'completed'
                      ...
                      : cellStatus.kind === 'downloading'
                        ? 'bg-cai-yellow/10 text-cai-yellow/80 border border-cai-yellow/20'
