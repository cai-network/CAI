...
<script lang="ts">
  ...
  function formatModelName(modelId: string | null | undefined): string {
    if (!modelId) return tr("Unknown Model");
  ...
  function formatStrategy(
    ...
    const shardLabel = sharding ?? tr("Unknown");
  ...
  function extractInstanceInferenceBackend(
    ...
    const [, shard] = getTaggedValue(firstShardWrapped);
  ...
  function describeInstance(instanceWrapped: unknown): {
    ...
    const modelId = extractInstanceModelId(instanceWrapped);
    const inferenceBackend = extractInstanceInferenceBackend(instanceWrapped);
  ...
  function resolveConversationInfo(
    ...
    const sharding =
      conversation.sharding ?? instanceDetails.sharding ?? tr("Unknown");
    const instanceType =
...
{#snippet sidebarContent()}
  ...
  <div class="p-4">
    <button
      ...
      class="w-full flex items-center justify-center gap-2 py-2.5 px-4 bg-transparent border border-cai-yellow/30 text-cai-yellow text-xs font-mono tracking-wider uppercase hover:border-cai-yellow/50 transition-all cursor-pointer"
    >
      <svg
        class="w-4 h-4"
        fill="none"
  ...
  <div class="px-4 py-3">
    <div class="relative">
      ...
      <input
        ...
        class="w-full bg-cai-black/40 border border-cai-medium-gray/30 rounded px-3 py-2 pl-9 text-xs text-white/90 placeholder:text-white/40 focus:outline-none focus:border-cai-yellow/30"
      />
    </div>
  </div>
  ...
  <div class="flex-1 overflow-y-auto">
    {#if filteredConversations.length > 0}
      <div class="py-2">
        <div class="px-4 py-2">
          <span
            class="text-xs text-cai-light-gray font-mono tracking-wider uppercase"
          >
            {searchQuery ? tr("SEARCH RESULTS") : tr("CONVERSATIONS")}
          </span>
        </div>
        ...
        {#each filteredConversations as conversation (conversation.id)}
          ...
          <div class="px-2">
            {#if editingId === conversation.id}
              <!-- Edit mode -->
              <div
                class="p-2 bg-transparent border border-cai-yellow/20 rounded mb-1"
              >
                <input
                  type="text"
                  bind:value={editingName}
                  onkeydown={handleEditKeydown}
                  class="w-full bg-cai-black/60 border border-cai-yellow/30 rounded px-2 py-1.5 text-xs text-cai-light-gray focus:outline-none focus:border-cai-yellow/50 mb-2"
                  autofocus
                />
                <div class="flex gap-2">
                  <button
                    onclick={handleSaveEdit}
                    class="flex-1 py-1.5 text-xs font-mono tracking-wider uppercase bg-transparent text-cai-yellow border border-cai-yellow/30 rounded hover:border-cai-yellow/50 cursor-pointer"
                  >
                    {tr("SAVE")}
                  </button>
                  <button
                    onclick={handleCancelEdit}
                    class="flex-1 py-1.5 text-xs font-mono tracking-wider uppercase bg-cai-medium-gray/20 text-cai-light-gray border border-cai-medium-gray/30 rounded hover:bg-cai-medium-gray/30 cursor-pointer"
                  >
                    {tr("CANCEL")}
                  </button>
                </div>
            ...
            {:else if deleteConfirmId === conversation.id}
              ...
              >
                <p class="text-xs text-red-400 mb-2">
                  {trf("Delete \"{name}\"?", { name: conversation.name })}
                </p>
                <div class="flex gap-2">
                  <button
            ...
            {:else}
              ...
              <div
                ...
                class="group w-full flex items-center justify-between p-2.5 rounded-lg mb-1 transition-all text-left cursor-pointer
                  ...
                  ? 'bg-cai-yellow/5 border border-cai-yellow/30'
              ...
              >
                ...
                >
                  <button
                    ...
                    class="p-1 text-cai-light-gray hover:text-cai-yellow transition-colors cursor-pointer"
                    title={tr("Rename")}
                  >
                    <svg
                      class="w-3 h-3"
                  ...
                  <button
                    ...
                    class="p-1 text-cai-light-gray hover:text-red-400 transition-colors cursor-pointer"
                    title={tr("Delete")}
                  >
                    <svg
                      class="w-3 h-3"
    ...
    {:else}
      ...
      >
        <div
          class="w-12 h-12 border border-cai-yellow/20 rounded-full flex items-center justify-center mb-3"
        >
          <svg
            class="w-6 h-6 text-cai-yellow/40"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
        ...
        >
          {searchQuery ? tr("NO RESULTS") : tr("NO CONVERSATIONS")}
        </p>
        <p class="text-sm text-white/50">
          {searchQuery ? tr("Try a different search") : tr("Start a new chat to begin")}
        </p>
      </div>
    {/if}
  </div>
  ...
  <div class="p-3 border-t border-cai-yellow/10">
      ...
      <div class="bg-red-500/10 border border-red-500/30 rounded p-2 mb-2">
        ...
        <div class="flex gap-2">
          ...
          <button
            ...
            class="flex-1 py-1.5 text-xs font-mono tracking-wider uppercase bg-cai-medium-gray/20 text-cai-light-gray border border-cai-medium-gray/30 rounded hover:bg-cai-medium-gray/30 transition-colors cursor-pointer"
          >
            {tr("CANCEL")}
          </button>
        </div>
    ...
    >
      <button
        ...
        class="p-1.5 rounded border border-cai-medium-gray/40 hover:border-cai-yellow/50 transition-colors cursor-pointer"
        title={tr("Toggle debug mode")}
      >
        <svg
          class="w-4 h-4 {debugEnabled
            ? 'text-cai-yellow'
            : 'text-cai-medium-gray'}"
          fill="currentColor"
          viewBox="0 0 24 24"
        >
          <path
      ...
      <div class="text-xs text-white/60 font-mono tracking-wider text-center">
        {conversationList.length} {conversationList.length === 1
          ? tr("conversation")
          : tr("conversations")}
      </div>
    </div>
...
{#if isMobileDrawer}
  ...
  {#if isOpen}
    ...
    <!-- Drawer panel -->
    <aside
      class="fixed left-0 top-0 bottom-0 w-72 bg-cai-dark-gray border-r border-cai-yellow/10 z-50 flex flex-col md:hidden"
    >
...
{:else}
  <!-- Desktop sidebar -->
  <aside
    class="flex flex-col h-full bg-cai-dark-gray border-r border-cai-yellow/10 {className}"
  >
    {@render sidebarContent()}
  </aside>
{/if}
