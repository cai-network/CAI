...
<script lang="ts">
  ...
  import { currentLanguage, tr } from "$lib/stores/i18n.svelte";
    ...
    return `
			<div class="code-block-wrapper">
				<div class="code-block-header">
					...
					<button type="button" class="copy-code-btn" data-code="${encodeURIComponent(text)}" title="${tr("Copy code")}">
  ...
  function preprocessLaTeX(text: string): string {
    ...
    processed = processed.replace(
      ...
      () => {
        ...
        htmlSnippets.set(
          placeholder,
          '<div class="latex-diagram-placeholder"><span class="latex-diagram-icon">📐</span><span class="latex-diagram-text">${tr("Diagram")}</span></div>',
        );
        htmlCounter++;
        return placeholder;
      },
    ...
    processed = processed.replace(
      ...
      () => {
        ...
        htmlSnippets.set(
          placeholder,
          '<div class="latex-diagram-placeholder"><span class="latex-diagram-icon">🖼️</span><span class="latex-diagram-text">${tr("Figure")}</span></div>',
        );
        htmlCounter++;
        return placeholder;
      },
    ...
    processed = processed.replace(
      ...
      (_, content) => {
        const html = `<div class="latex-proof"><div class="latex-proof-header">${tr("Proof")}</div><div class="latex-proof-content">${content}</div></div>`;
        const placeholder = `${HTML_PLACEHOLDER_PREFIX}${htmlCounter}END`;
            ...
            return `
							<div class="math-display-wrapper">
								<div class="math-display-header">
									...
									<button type="button" class="copy-math-btn" data-math-source="${encodeURIComponent(content)}" title="${tr("Copy LaTeX source")}">
...
<style>
  ...
  .markdown-content :global(li::marker) {
    color: var(--cai-light-gray, #9ca3af);
  }

  /* Blockquotes */
  .markdown-content :global(blockquote) {
    border-left: 3px solid var(--cai-yellow, #7dd3fc);
    padding: 0.5rem 1rem;
    margin: 1rem 0;
  ...
  /* Syntax highlighting - dark theme matching cai style */
