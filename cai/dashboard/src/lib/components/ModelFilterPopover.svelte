...
<div
  class="filter-popover absolute right-0 top-full mt-2 w-64 bg-cai-dark-gray border border-cai-yellow/10 rounded-lg shadow-xl z-20"
...
>
  <div class="p-3 space-y-4">
    <!-- Capabilities -->
    <div>
      <h4 class="text-xs font-mono text-white/50 mb-2">{tr("Capabilities")}</h4>
        ...
        {#each availableCapabilities as cap}
          ...
          <button
            ...
            class="px-2 py-1 text-xs font-mono rounded transition-colors {isSelected
              ? 'bg-cai-yellow/20 text-cai-yellow border border-cai-yellow/30'
          ...
          >
            ...
            {:else if cap.id === "image_edit"}
              ...
              >
            {/if}
            <span class="ml-1">{tr(cap.label)}</span>
          </button>
        {/each}
      </div>
    </div>
    ...
    <div>
      <h4 class="text-xs font-mono text-white/50 mb-2">{tr("Availability")}</h4>
      <div class="flex flex-wrap gap-1.5">
        <button
          type="button"
        ...
        >
          ...
          <span class="ml-1">{tr("Downloaded")}</span>
        </button>
        <button
          type="button"
    ...
    <!-- Size range -->
    <div>
      <h4 class="text-xs font-mono text-white/50 mb-2">{tr("Model Size")}</h4>
      <div class="flex flex-wrap gap-1.5">
