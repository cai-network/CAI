...
<script lang="ts">
  ...
  // GGUF repos tend to have readable short names, so shorten those in the list.
  ...
  <div class="flex-1 min-w-0">
    <div class="flex items-center gap-2">
      ...
      {#if downloadedOnNodes.length > 0}
        <span
          class="flex-shrink-0"
          title={trf("Downloaded on {nodes}", {
            nodes: downloadedOnNodes.join(", "),
          })}
        >
          <svg
            class="w-4 h-4"
    ...
    <div class="flex items-center gap-3 mt-0.5 text-xs text-white/40">
      ...
      <span
        ...
        title={tr("Downloads in the last 30 days")}
      >
        <svg
          class="w-3 h-3"
          fill="none"
      ...
      <span
        class="flex items-center gap-1 shrink-0"
        title={tr("Community likes on Hugging Face")}
      >
        <svg
          class="w-3 h-3"
          fill="none"
  ...
  <div class="flex items-center gap-2 shrink-0">
    {#if isAdded}
      <button
        type="button"
        onclick={onSelect}
        class="px-3 py-1.5 text-xs font-mono tracking-wider uppercase bg-cai-yellow/10 text-cai-yellow border border-cai-yellow/30 hover:bg-cai-yellow/20 transition-colors rounded cursor-pointer"
      >
        {tr("Select")}
      </button>
    {:else}
