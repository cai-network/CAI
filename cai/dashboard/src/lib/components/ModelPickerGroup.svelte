...
<script lang="ts">
  ...
  function timeAgo(ts: number): string {
    ...
    if (minutes < 60) return trf("{count}m ago", { count: minutes });
    const hours = Math.floor(minutes / 60);
    if (hours < 24) return trf("{count}h ago", { count: hours });
    const days = Math.floor(hours / 24);
    return trf("{count}d ago", { count: days });
  }
...
>
  ...
  <div
    ...
    anyVariantHasInstance
      ...
      : 'hover:bg-white/5 cursor-pointer'} {isMainSelected
      ? 'bg-cai-yellow/10 border-l-2 border-cai-yellow'
  ...
  >
        ...
        {#each group.capabilities.filter((c) => c !== "text") as cap}
          {#if cap === "thinking"}
            <svg
              ...
              stroke-width="1.5"
            >
              <title>{tr("Supports Thinking")}</title>
              <path
          ...
          {:else if cap === "code"}
            <svg
              ...
              stroke-width="1.5"
            >
              <title>{tr("Supports code generation")}</title>
              <path
          ...
          {:else if cap === "vision"}
            <svg
              ...
              stroke-width="1.5"
            >
              <title>{tr("Supports image input")}</title>
              <path
          ...
          {:else if cap === "image_gen"}
            <svg
              ...
              stroke-width="1.5"
            >
              <title>{tr("Supports image generation")}</title>
              <rect
                x="3"
                y="3"
                width="18"
          ...
          {:else if cap === "image_edit"}
            <svg
              ...
              stroke-width="1.5"
            >
              <title>{tr("Supports image editing")}</title>
              <path
    ...
    {#if group.hasMultipleVariants}
      ...
      >
        {group.variants.length} {tr("variants")}{#if sizes.length >= 2}{" "}({formatSize(
            sizes[0],
    ...
    {#if groupDownloadStatus && groupDownloadStatus.nodeIds.length > 0}
      <span
        ...
        title={groupDownloadStatus.available
          ? trf("Ready - downloaded on {nodes}", {
              nodes: groupDownloadStatus.nodeNames.join(", "),
            })
          : trf("Downloaded on {nodes} (may need more nodes)", {
              nodes: groupDownloadStatus.nodeNames.join(", "),
            })}
      >
        <svg
    ...
    {#if groupInstanceStatus}
      {#if groupInstanceStatus.status === "READY" || groupInstanceStatus.status === "LOADED" || groupInstanceStatus.status === "RUNNING"}
        <span class="flex-shrink-0" title={tr("Running")}>
          <svg
      ...
      {:else if groupInstanceStatus.status === "DOWNLOADING"}
        <span class="flex-shrink-0 animate-pulse" title={tr("Downloading")}>
          <svg
      ...
      {:else if groupInstanceStatus.status === "LOADING" || groupInstanceStatus.status === "WARMING UP"}
        <span class="flex-shrink-0 animate-pulse" title={tr("Loading")}>
          <svg
    ...
    {#if isMainSelected}
      <svg
        class="w-4 h-4 text-cai-yellow flex-shrink-0"
        viewBox="0 0 24 24"
        fill="currentColor"
      >
    ...
    <div class="bg-black/20 border-t border-white/5">
      {#each group.variants as variant}
        ...
        >
          ...
          >
            {variant.quantization || tr("default")}
          </span>
          ...
          {#if downloadStatusMap?.get(variant.id)}
            ...
            {#if variantDl}
              <span
                class="flex-shrink-0"
                title={trf("Downloaded on {nodes}", { nodes: variantDl.nodeNames.join(", ") })}
              >
                <svg
                  class="w-3.5 h-3.5"
                  viewBox="0 0 24 24"
