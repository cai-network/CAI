...
<script lang="ts" module>
  ...
  export function pickAutoModel(
    ...
    const runtimeModels = modelList.filter(
      (model) =>
...
<script lang="ts">
  ...
  const candidateModels = $derived.by(() => {
  ...
  function catalogModelKey(model: ChatModelInfo): string {
    return [
  ...
  function preferCatalogModel(
    current: ChatModelInfo | undefined,
    candidate: ChatModelInfo,
  ): ChatModelInfo {
    if (!current) return candidate;
    const currentIsNetwork = current.id.startsWith("cai-network/");
    const candidateIsNetwork = candidate.id.startsWith("cai-network/");
    if (currentIsNetwork !== candidateIsNetwork) {
      return candidateIsNetwork ? current : candidate;
    }
    const currentSize = current.storage_size_megabytes || 0;
    const candidateSize = candidate.storage_size_megabytes || 0;
  ...
  const compactCatalogModels = $derived.by(() => {
    const deduped = new Map<string, ChatModelInfo>();
    for (const model of candidateModels) {
      const key = catalogModelKey(model);
      deduped.set(key, preferCatalogModel(deduped.get(key), model));
    }
    return [...deduped.values()].sort((a, b) => {
      const aFits = fitsInMemory(a);
      const bFits = fitsInMemory(b);
  ...
  const showCompactCatalog = $derived(compactCatalogModels.length > 0 && compactCatalogModels.length <= 6);
  ...
  function pickBestVariant(baseModel: string): ChatModelInfo | null {
    const variants = candidateModels
  ...
  const recommendations = $derived.by((): CategoryRecommendation[] => {
    return [
      ...
      {
        category: "writing",
        label: tr("Best for Writing"),
        model: pickFromRanking(WRITING_RANKING),
        tooltip: tr(
          "Ranked by creative writing quality and instruction following",
        ),
      },
      {
        category: "agentic",
        label: tr("Best Agentic"),
        model: pickFromRanking(AGENTIC_RANKING),
        tooltip: tr("Ranked by reasoning, planning, and tool-use capability"),
      },
      {
        category: "biggest",
        label: tr("Biggest"),
        model: pickBiggest(),
        tooltip: tr("Largest model that fits in your available memory"),
      },
    ];
  });
  ...
  function compactModelTitle(model: ChatModelInfo): string {
    return model.base_model || model.name || model.id;
  }
...
<div class="flex flex-col items-center justify-center gap-6 {className}">
  ...
  <div class="text-center">
    <p class="text-xs text-cai-light-gray uppercase tracking-[0.2em] mb-1">
      {tr("Recommended GGUF models for your")}
    </p>
    <p class="text-sm text-white font-mono tracking-wide">{clusterLabel}</p>
  </div>

  {#if showCompactCatalog}
    <div class="grid grid-cols-1 gap-3 w-full max-w-md">
      {#each compactCatalogModels as model}
        <button
          type="button"
          onclick={() => onSelect(model.id, "catalog")}
          class="group flex items-center justify-between gap-3 p-4 rounded-lg border border-cai-medium-gray/50 bg-cai-dark-gray/50 hover:border-cai-yellow/40 hover:bg-cai-dark-gray transition-all duration-200 cursor-pointer text-left"
        >
          <div class="min-w-0">
            <p class="text-sm text-white font-mono truncate">
              {compactModelTitle(model)}
            </p>
            <p class="text-xs text-cai-light-gray/60 font-mono mt-0.5 truncate">
              {formatSize(model.storage_size_megabytes)}
              {#if model.quantization}
                <span class="text-cai-light-gray/40"
                  >&middot; {model.quantization}</span
                >
              {/if}
            </p>
          </div>
          <div class="flex-shrink-0">
            <span
              class={fitsInMemory(model)
                ? "block w-2.5 h-2.5 rounded-full bg-cai-yellow"
                : "block w-2.5 h-2.5 rounded-full bg-cai-light-gray/35"}
            ></span>
          </div>
        </button>
      {/each}
      ...
      {#each recommendations as rec}
        {#if rec.model}
          <button
            ...
            class="group relative flex flex-col items-start gap-2 p-4 rounded-lg border border-cai-medium-gray/50 bg-cai-dark-gray/50 hover:border-cai-yellow/40 hover:bg-cai-dark-gray transition-all duration-200 cursor-pointer text-left"
          >
            <!-- Category icon + label -->
            <div class="flex items-center gap-2 w-full">
              <svg
                class="w-4 h-4 text-cai-yellow/70 group-hover:text-cai-yellow transition-colors flex-shrink-0"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                stroke-width="1.5"
              ...
              <span
                class="text-xs font-mono uppercase tracking-wider text-cai-light-gray group-hover:text-white transition-colors"
              >
                {rec.label}
              </span>
              <!-- Info tooltip -->
              <div class="ml-auto flex-shrink-0">
                <span
                  role="img"
                  aria-label={rec.tooltip}
                  class="text-cai-light-gray/40 hover:text-cai-light-gray transition-colors cursor-help inline-flex"
            ...
            <div class="w-full">
              ...
              <p class="text-xs text-cai-light-gray/60 font-mono mt-0.5">
        ...
        {:else}
          ...
          <div
            class="flex flex-col items-start gap-2 p-4 rounded-lg border border-cai-medium-gray/30 bg-cai-dark-gray/30 opacity-50"
          >
            <div class="flex items-center gap-2">
              <svg
                class="w-4 h-4 text-cai-light-gray/40 flex-shrink-0"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                stroke-width="1.5"
              ...
              <span
                class="text-xs font-mono uppercase tracking-wider text-cai-light-gray/50"
                >{rec.label}</span
              >
            </div>
            <p class="text-xs text-cai-light-gray/40 font-mono">
              {tr("No model fits")}
            </p>
          </div>
        {/if}
  ...
  <button
    type="button"
    onclick={onAddModel}
    class="flex items-center gap-2 px-4 py-2 text-xs font-mono uppercase tracking-wider text-cai-light-gray hover:text-cai-yellow border border-cai-medium-gray/30 hover:border-cai-yellow/30 rounded-lg transition-all duration-200 cursor-pointer"
  >
    <svg
      class="w-3.5 h-3.5"
      fill="none"
  ...
  <!-- Auto hint -->
  <p class="text-xs text-cai-light-gray/40 font-mono tracking-wide text-center">
    {tr("Or just start typing - we'll pick the best model automatically")}
  </p>
</div>
...
{#if hoveredTooltip && tooltipAnchor}
  ...
  {#if rec}
    <div
      class="fixed z-[9999] px-3 py-2 bg-cai-black border border-cai-medium-gray/50 rounded text-xs text-cai-light-gray whitespace-nowrap shadow-lg pointer-events-none"
