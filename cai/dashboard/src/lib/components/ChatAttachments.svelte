...
{#if files.length > 0}
  <div class="flex flex-wrap gap-2 mb-3 px-1">
    {#each files as file (file.id)}
      <div
        class="group relative flex items-center gap-2 bg-cai-dark-gray/80 border border-cai-yellow/30 rounded px-2.5 py-1.5 text-xs font-mono transition-all hover:border-cai-yellow/50 hover:shadow-[0_0_10px_rgba(125,211,252,0.1)]"
      >
        ...
        {#if file.preview && getFileCategory(file.type, file.name) === "image"}
          <img
            src={file.preview}
            alt={file.name}
            class="w-8 h-8 object-cover rounded border border-cai-yellow/20"
          />
        {:else}
        ...
        <div class="flex flex-col min-w-0">
          <span
            class="text-cai-yellow truncate max-w-[120px]"
            title={file.name}
          >
            {truncateName(file.name)}
          </span>
          <span class="text-cai-light-gray text-xs">
        ...
        {#if !readonly && onRemove}
          <button
            ...
            class="ml-1 w-4 h-4 flex items-center justify-center text-cai-light-gray hover:text-red-400 transition-colors cursor-pointer"
