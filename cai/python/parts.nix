...
{ inputs, ... }:
{
  perSystem =
    ...
    let
      ...
      CAIOverlay = final: prev: {
        # Replace workspace cai_pyo3_bindings with Nix-built wheel.
        # Preserve passthru so mkVirtualEnv can resolve dependency groups.
        # Copy .pyi stub + py.typed marker so basedpyright can find the types.
        cai-pyo3-bindings = pkgs.stdenv.mkDerivation {
          pname = "cai-pyo3-bindings";
          version = "0.1.0";
          src = self'.packages.cai_pyo3_bindings;
          # Install from pre-built wheel
          nativeBuildInputs = [ final.pyprojectWheelHook ];
          dontStrip = true;
          passthru = prev.cai-pyo3-bindings.passthru or { };
          postInstall = ''
            local siteDir=$out/${final.python.sitePackages}/cai_pyo3_bindings
            cp ${inputs.self}/rust/cai_pyo3_bindings/cai_pyo3_bindings.pyi $siteDir/
      ...
      CAIDeps = removeAttrs workspace.deps.default [ "cai-bench" ];

      CAIVenv = (pythonSet.mkVirtualEnv "CAI-env" CAIDeps).overrideAttrs {
        venvIgnoreCollisions = venvCollisionPaths;
      };

      # Virtual environment with dev dependencies for testing
      testVenv = (pythonSet.mkVirtualEnv "CAI-test-env" (
        CAIDeps // {
          cai = [ "dev" ]; # Include pytest, pytest-asyncio, pytest-env
        }
      )).overrideAttrs {
        venvIgnoreCollisions = venvCollisionPaths;
      };
      ...
      mkPythonScript = name: path: pkgs.writeShellApplication {
        inherit name;
        runtimeInputs = [ CAIVenv ];
        runtimeEnv = {
          CAI_DASHBOARD_DIR = self'.packages.dashboard;
          CAI_RESOURCES_DIR = inputs.self + /resources;
        };
        text = ''exec python ${path} "$@"'';
      };

      benchVenv = pythonSet.mkVirtualEnv "cai-bench-env" {
        cai-bench = [ ];
      };
      ...
      CAIPackage = pkgs.runCommand "cai"
        {
        ...
        ''
          ...
          makeWrapper ${CAIVenv}/bin/cai $out/bin/cai \
            --set CAI_DASHBOARD_DIR ${self'.packages.dashboard} \
            --set CAI_RESOURCES_DIR ${inputs.self + /resources} \
      ...
      packages = lib.optionalAttrs pkgs.stdenv.hostPlatform.isDarwin
        {
          ...
          CAI-test-env = testVenv;
        } // {

        inherit python;

        cai-bench = mkBenchScript "cai-bench" (inputs.self + /bench/cai_bench.py);
        cai-eval = mkBenchScript "cai-eval" (inputs.self + /bench/cai_eval.py);
        cai-eval-tool-calls = mkBenchScript "cai-eval-tool-calls" (inputs.self + /bench/eval_tool_calls.py);
        cai-get-all-models-on-cluster = mkSimplePythonScript "cai-get-all-models-on-cluster" (inputs.self + /tests/get_all_models_on_cluster.py);
      };
