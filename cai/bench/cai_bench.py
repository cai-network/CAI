...
# type: ignore
#!/usr/bin/env python3
"""Benchmark CAI throughput across placement previews."""
...
class SystemMetricsSampler:
    def __init__(self, client: CaiClient, node_ids: list[str], interval_s: float = 1.0):
        self._client = client
...
def main() -> int:
    ap = argparse.ArgumentParser(
        prog="cai-bench",
        description="Benchmark CAI model throughput across placement previews.",
    )
    ...
    if tokenizer is None:
        raise RuntimeError("[cai-bench] tokenizer load failed")

    try:
        prompt_sizer = PromptSizer(tokenizer)
        logger.debug(f"[cai-bench] loaded tokenizer: {full_model_id} for prompt sizer")
    except Exception:
        logger.error("[cai-bench] tokenizer usable but prompt sizing failed")
        raise
    ...
    logger.debug(f"cai-bench model: short_id={short_id} full_id={full_model_id}")
    ...
    for preview in selected:
        ...
        if not args.no_system_metrics:
            ...
            sampler = SystemMetricsSampler(
                CaiClient(args.host, args.port, timeout_s=30),
                nids,
