...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="cai-eval-tool-calls",
        description="Multi-API tool-calling eval for CAI",
...
""",
    ...
    cai = CaiClient(args.host, args.port, timeout_s=args.timeout)
    _short_id, full_model_id = resolve_model_short_id(cai, args.model)

    selected = settle_and_fetch_placements(
        cai, full_model_id, args, settle_timeout=args.settle_timeout
    )
    if not selected:
    ...
    cai.request_json("POST", "/instance", body={"instance": instance})
    try:
        wait_for_instance_ready(cai, instance_id)
    except (RuntimeError, TimeoutError) as e:
        print(f"Failed to initialize placement: {e}", file=sys.stderr)
        with contextlib.suppress(CaiHttpError):
            cai.request_json("DELETE", f"/instance/{instance_id}")
        sys.exit(1)

    time.sleep(1)
    cluster_snapshot = capture_cluster_snapshot(cai)
    ...
    try:
        ...
        try:
            ...
            if e.status != 404:
                raise
        wait_for_instance_gone(cai, instance_id)
