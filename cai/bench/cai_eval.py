...
# type: ignore
#!/usr/bin/env python3
"""Quality evaluation for CAI — matches Artificial Analysis methodology.

Runs LLM benchmarks against CAI's OpenAI-compatible API using the same
...
Usage:
  uv run python cai_eval.py --model <model-id> --tasks gpqa_diamond
  uv run python cai_eval.py --model <model-id> --tasks humaneval,livecodebench --limit 50
  uv run python cai_eval.py --model <model-id> --tasks gpqa_diamond --compare-concurrency 1,4
...
def main() -> int:
    ap = argparse.ArgumentParser(
        prog="cai-eval",
        description="Quality evaluation for CAI — matches Artificial Analysis methodology.",
    )
    ...
    ap.add_argument(
        "--skip-instance-setup",
        action="store_true",
        help="Skip CAI instance management (assumes model is already running).",
    )
    ...
    # Instance management
    client = CaiClient(args.host, args.port, timeout_s=args.timeout)
    ...
    if not args.skip_instance_setup:
        ...
        try:
            ...
            with contextlib.suppress(CaiHttpError):
