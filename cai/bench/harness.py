...
class CaiHttpError(RuntimeError):
...
class CaiClient:
    ...
    def request_json(
        ...
        try:
            ...
            if resp.status >= 400:
                raise CaiHttpError(resp.status, resp.reason, text[:300])
...
def wait_for_instance_ready(
    client: CaiClient, instance_id: str, timeout: float = 24000.0
) -> None:
...
def wait_for_instance_gone(
    client: CaiClient, instance_id: str, timeout: float = 3.0
) -> None:
    start_time = time.time()
    while time.time() - start_time < timeout:
        try:
...
def capture_cluster_snapshot(client: CaiClient) -> dict[str, Any]:
...
def resolve_model_short_id(
    client: CaiClient, model_arg: str, *, force_download: bool = False
) -> tuple[str, str]:
...
def fetch_and_filter_placements(
    client: CaiClient, full_model_id: str, args: argparse.Namespace
) -> list[dict[str, Any]]:
...
def add_common_instance_args(ap: argparse.ArgumentParser) -> None:
    ap.add_argument(
        "--host", default=os.environ.get("CAI_HOST") or os.environ.get("CAI_HOST", "localhost")
    )
    ap.add_argument(
        "--port",
        type=int,
        default=int(os.environ.get("CAI_PORT") or os.environ.get("CAI_PORT", "52415")),
    )
    ap.add_argument("--model", required=True, help="Model short id or huggingface id")
    ap.add_argument(
        "--force-download",
        action="store_true",
        help="If model not in /models, add it from HuggingFace via CAI and download.",
    )
    ap.add_argument(
        "--max-nodes",
        type=int,
