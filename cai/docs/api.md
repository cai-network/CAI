...
-->
# cai API – Technical Reference

This document describes the REST API exposed by the **cai** service, as implemented in:

`src/CAI/master/api.py`
...
cai provides Ollama API compatibility for tools like OpenWebUI.
...
* `model`: string, required - Image editing model ID (e.g., `CAIlabs/FLUX.1-Kontext-dev`)
...
cai provides multiple API-compatible interfaces:
...
Existing OpenAI, Claude, or Ollama clients can be pointed to cai by changing the base URL.
