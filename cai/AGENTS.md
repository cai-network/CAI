...
cai is a distributed AI inference system that connects multiple devices into a cluster. It enables running large language models across multiple machines using MLX as the inference backend and libp2p for peer-to-peer networking.

## Build & Run Commands

```bash
# Build the dashboard (required before running cai)
cd dashboard && npm install && npm run build && cd ..

# Run cai (starts both master and worker with API at http://localhost:52415)
uv run cai

# Run with verbose logging
uv run cai -v   # or -vv for more verbose
...
uv run pytest src/CAI/shared/tests/test_election.py

# Run a specific test function
uv run pytest src/CAI/shared/tests/test_election.py::test_function_name
...
### Node Composition
A single cai `Node` (src/CAI/main.py) runs multiple components:
- **Router**: libp2p-based pub/sub messaging via Rust bindings (cai_pyo3_bindings)
...
### Message Flow
Components communicate via typed pub/sub topics (src/CAI/routing/topics.py):
...
- `State` (src/CAI/shared/types/state.py): Immutable state object
- `apply()` (src/CAI/shared/apply.py): Pure function that applies events to state
- Master indexes events and broadcasts; workers apply indexed events

### Key Type Hierarchy
- `src/CAI/shared/types/`: Pydantic models for all shared types
...
- `cai_pyo3_bindings`: PyO3 bindings exposing Rust to Python
...
Tests use pytest-asyncio with `asyncio_mode = "auto"`. Tests are in `tests/` subdirectories alongside the code they test. The `CAI_TESTS=1` env var is set during tests.

## Dashboard UI Testing & Screenshots

### Building and Running the Dashboard
```bash
# Build the dashboard (must be done before running cai)
cd dashboard && npm install && npm run build && cd ..

# Start cai (serves the dashboard at http://localhost:52415)
uv run cai &
...
(async () => {
  ...
  await page.evaluate(() => {
    localStorage.setItem('CAI-recent-models', JSON.stringify([
...
2. **Post PR comment** referencing the raw image URL (uses permanent commit SHA so images survive deletion):
   ```bash
   gh pr comment <PR_NUMBER> --body "![Screenshot](https://raw.githubusercontent.com/CAI-explore/CAI/${COMMIT_SHA}/screenshot.png)"
   ```
