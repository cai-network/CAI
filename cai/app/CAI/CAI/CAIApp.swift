...
struct CAIApp: App {
    @StateObject private var controller: CAIProcessController
    ...
    init() {
        let controller = CAIProcessController()
    ...
    private func enableLaunchAtLoginIfNeeded() {
        ...
        do {
            ...
            Logger().error(
                "Failed to register cai for launch at login: \(error.localizedDescription)")
        }
    }

}

/// Helper for managing cai's launch-at-login registration
enum LaunchAtLoginHelper {
    private static let logger = Logger(subsystem: "io.cai.cai", category: "LaunchAtLogin")

    /// Unregisters cai from launching at login
    static func disable() {
        guard SMAppService.mainApp.status == .enabled else { return }
        do {
            try SMAppService.mainApp.unregister()
            logger.info("Unregistered cai from launch at login")
        } catch {
            logger.error(
                "Failed to unregister cai from launch at login: \(error.localizedDescription, privacy: .public)"
            )
        }
    }
}
...
final class SparkleUpdater: NSObject, ObservableObject {
    ...
    private let delegateProxy: CAIUpdaterDelegate
    private let notificationDelegate = CAINotificationDelegate()
    private var periodicCheckTask: Task<Void, Never>?

    init(processController: CAIProcessController) {
        let proxy = CAIUpdaterDelegate(processController: processController)
        delegateProxy = proxy
...
private final class CAIUpdaterDelegate: NSObject, SPUUpdaterDelegate {
    private weak var processController: CAIProcessController?
    ...
    nonisolated func updater(_ updater: SPUUpdater, didFindValidUpdate item: SUAppcastItem) {
        showNotification(
            ...
            body: "cai \(item.displayVersionString) is ready to install."
        )
    }
    ...
    nonisolated private func showNotification(title: String, body: String) {
        ...
        let request = UNNotificationRequest(
            identifier: "CAI-update-\(UUID().uuidString)",
            content: content,
            trigger: nil
        )
...
private final class CAINotificationDelegate: NSObject, UNUserNotificationCenterDelegate {
