...
struct ContentView: View {
    @EnvironmentObject private var controller: CAIProcessController
    ...
    private var shouldShowLocalNetworkWarning: Bool {
        // Show warning if local network is not working and cai is running.
    ...
    /// Shows TB bridge status for all nodes from cai cluster state
    ...
    private func openGitHubIssue() {
        ...
        bodyParts.append("- cai Version: \(buildTag) (\(buildCommit))")
        bodyParts.append("")
        ...
        var components = URLComponents(string: "https://github.com/CAI-explore/CAI/issues/new")!
    ...
    private func showUninstallConfirmationAlert() {
        let alert = NSAlert()
        alert.messageText = "Uninstall cai"
        alert.informativeText = """
            This will remove cai and all its system components:

            • Network configuration daemon
            • Launch at login registration
            • cai network location
    ...
    private var buildTag: String {
        Bundle.main.infoDictionary?["CAIBuildTag"] as? String ?? "unknown"
    }

    private var buildCommit: String {
        Bundle.main.infoDictionary?["CAIBuildCommit"] as? String ?? "unknown"
    }
}
