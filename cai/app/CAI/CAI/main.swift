...
/// Command line options for the cai app
enum CLICommand {
    case install
    case uninstall
    case help
...
func printUsage() {
    let programName = (CommandLine.arguments.first as NSString?)?.lastPathComponent ?? "cai"
    print(
        """
        Usage: \(programName) [OPTIONS]

        Options:
          --install     Install cai network configuration (requires root)
          --uninstall   Uninstall cai network configuration (requires root)
