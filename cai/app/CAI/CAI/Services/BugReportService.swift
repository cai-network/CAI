...
struct BugReportService {
    ...
    func sendReport(
        ...
        // Extract cluster TB bridge status from cai state
        ...
        return BugReportOutcome(
            success: true, message: "Bug Report sent. Thank you for helping to improve cai 1.0.")
    }
    ...
    {
        guard
            let endpointString = bundle.infoDictionary?["CAIBugReportPresignedUrlEndpoint"]
                as? String
        else {
    ...
    private func makeReportJson(
        ...
        var payload: [String: Any] = [
            ...
            "system": system,
            "CAI_version": cai.version as Any,
            "CAI_commit": cai.commit as Any,
    ...
    /// Extracts cluster-wide Thunderbolt Bridge status from cai state JSON
    ...
    private func readCaiMetadata(bundle: Bundle = .main) -> (version: String?, commit: String?) {
        let info = bundle.infoDictionary ?? [:]
        let tag = info["CAIBuildTag"] as? String
        let short = info["CFBundleShortVersionString"] as? String
        let version = [tag, short]
            .compactMap { $0?.trimmingCharacters(in: .whitespacesAndNewlines) }
            .first { !$0.isEmpty }
        let commit = (info["CAIBuildCommit"] as? String)?
