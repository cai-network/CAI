...
enum NetworkSetupHelper {
    private static let logger = Logger(subsystem: "io.cai.cai", category: "NetworkSetup")
    private static let daemonLabel = "io.cai.networksetup"
    private static let scriptDestination =
        "/Library/Application Support/CAI/disable_bridge.sh"
    // Legacy script path from older versions
    private static let legacyScriptDestination =
        "/Library/Application Support/CAI/disable_bridge_enable_dhcp.sh"
    private static let plistDestination = "/Library/LaunchDaemons/io.cai.networksetup.plist"
    ...
    private static let setupScript = """
        ...
        networksetup -listlocations | grep -q cai || {
          networksetup -createlocation cai
        }

        networksetup -switchtolocation cai
              ...
              case "$name" in
                ...
                "Thunderbolt "*)
                  networksetup -listallnetworkservices \\
                    | grep -q "cai $name" \\
                      || networksetup -createnetworkservice "cai $name" "$name" 2>/dev/null \\
                      || continue
                  networksetup -setdhcp "cai $name"
                        ;;
                *)
                  networksetup -listallnetworkservices \\
                    | grep -q "$name" \\
    ...
    static func promptAndInstallIfNeeded() {
        ...
        Task.detached(priority: .utility) {
            ...
            let shouldInstall = await MainActor.run {
                let alert = NSAlert()
                alert.messageText = "cai Network Configuration"
                alert.informativeText =
                    "cai needs to install a system service to configure local networking. This will disable Thunderbolt Bridge (preventing packet storms) and install a Network Location.\n\nYou will be prompted for your password."
    ...
    /// Removes all cai network setup components from the system.
    ...
    static func uninstall() throws {
        ...
        logger.info("cai network setup components removed successfully")
    }

    /// Checks if there are any cai network components installed that need cleanup
    ...
    private static func makeUninstallScript() -> String {
        ...
        networksetup -listlocations 2>/dev/null | grep -q '^cai$' && {
          networksetup -deletelocation cai >/dev/null 2>&1 || true
        } || true
        ...
        echo "cai network components removed successfully"
        """
    }
