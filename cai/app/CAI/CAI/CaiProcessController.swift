...
private let customNamespaceKey = "CAICustomNamespace"
private let hfTokenKey = "CAIHFToken"
private let hfEndpointKey = "CAIHFEndpoint"
private let enableImageModelsKey = "CAIEnableImageModels"
private let offlineModeKey = "CAIOfflineMode"
private let fastSynchEnabledKey = "CAIFastSynchEnabled"
private let onboardingCompletedKey = "CAIOnboardingCompleted"
private let defaultModelsDirKey = "CAIDefaultModelsDir"
private let additionalModelsDirsKey = "CAIAdditionalModelsDirs"
private let readOnlyModelsDirsKey = "CAIReadOnlyModelsDirs"
private let customEnvironmentVariablesKey = "CAICustomEnvironmentVariables"

/// A user-defined environment variable that is injected into the cai child
...
@MainActor
final class CAIProcessController: ObservableObject {
    ...
    static let CAIDirectoryURL: URL = {
        URL(fileURLWithPath: NSHomeDirectory()).appendingPathComponent(".cai")
    }()
    ...
    /// Fires once when cai transitions to `.running` for the very first time (fresh install).
    ...
    func launch() {
        do {
            ...
            let executableURL = runtimeURL.appendingPathComponent("cai")

            let child = Process()
            child.executableURL = executableURL
            let CAIHomeURL = Self.CAIDirectoryURL
            try? FileManager.default.createDirectory(
                at: CAIHomeURL, withIntermediateDirectories: true
            )
            child.currentDirectoryURL = CAIHomeURL
    ...
    private func resolveRuntimeDirectory() throws -> URL {
        ...
        if let override = ProcessInfo.processInfo.environment["CAI_RUNTIME_DIR"] {
        ...
        if let resourceRoot = Bundle.main.resourceURL {
            let bundled = resourceRoot.appendingPathComponent("cai", isDirectory: true)
        ...
        let repoCandidate = URL(fileURLWithPath: fileManager.currentDirectoryPath)
            .appendingPathComponent("dist/cai", isDirectory: true)
        if fileManager.fileExists(atPath: repoCandidate.path) {
            return repoCandidate
        }

        throw RuntimeError("Unable to locate the packaged cai runtime.")
    }

    private func makeEnvironment(for runtimeURL: URL) -> [String: String] {
        var environment = ProcessInfo.processInfo.environment
        environment["CAI_RUNTIME_DIR"] = runtimeURL.path
        environment["CAI_LIBP2P_NAMESPACE"] = computeNamespace()
        if !hfToken.isEmpty {
        ...
        if enableImageModels {
            environment["CAI_ENABLE_IMAGE_MODELS"] = "true"
        }
        if offlineMode {
            environment["CAI_OFFLINE"] = "true"
        }
        environment["CAI_FAST_SYNCH"] = fastSynchEnabled ? "true" : "false"
        ...
        if !trimmedDefaultModelsDir.isEmpty {
            environment["CAI_DEFAULT_MODELS_DIR"] = trimmedDefaultModelsDir
        }
        ...
        if !trimmedAdditionalModelsDirs.isEmpty {
            environment["CAI_MODELS_DIRS"] = trimmedAdditionalModelsDirs
        }
        let trimmedReadOnlyModelsDirs = readOnlyModelsDirs.trimmingCharacters(in: .whitespaces)
        if !trimmedReadOnlyModelsDirs.isEmpty {
            environment["CAI_MODELS_READ_ONLY_DIRS"] = trimmedReadOnlyModelsDirs
        }
    ...
    private func buildTag() -> String {
        if let tag = Bundle.main.infoDictionary?["CAIBuildTag"] as? String, !tag.isEmpty {
            return tag
        }
