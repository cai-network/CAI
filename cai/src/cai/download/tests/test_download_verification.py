...
from cai.shared.types.worker.downloads import FileListEntry, RepoFileDownloadProgress
...
class TestFileVerification:
    ...
    async def test_redownload_when_file_size_changes_upstream(
        ...
        with (
            patch(
                "cai.download.download_utils.file_meta",
                new_callable=AsyncMock,
                return_value=(remote_size, remote_hash),
            ) as mock_file_meta,
            patch(
                "cai.download.download_utils.create_http_session"
            ...
            with patch(
                "cai.download.download_utils.calc_hash",
                new_callable=AsyncMock,
...
class TestFileListCache:
    ...
    async def test_fetch_fresh_and_update_cache(
        ...
        with (
            patch("cai.download.download_utils.CAI_MODELS_DIRS", (models_dir,)),
            patch("cai.download.download_utils.CAI_DEFAULT_MODELS_DIR", models_dir),
            patch(
                "cai.download.download_utils.fetch_file_list_with_retry",
                new_callable=AsyncMock,
