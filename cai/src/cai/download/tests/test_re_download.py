...
from cai.download.download_utils import RepoDownloadProgress
from cai.download.impl_shard_downloader import SingletonShardDownloader
from cai.download.shard_downloader import ShardDownloader
...
from cai.shared.types.events import Event, NodeDownloadProgress
from cai.shared.types.memory import Memory
from cai.shared.types.worker.downloads import DownloadCompleted
from cai.shared.types.worker.shards import PipelineShardMetadata, ShardMetadata
...
class ExistingPendingFakeDownloader(FakeShardDownloader):
    async def get_shard_download_status(
        self,
    ) -> AsyncIterator[tuple[Path, RepoDownloadProgress]]:
        shard = _make_shard()
...
async def test_re_download_after_delete_completes() -> None:
    ...
    with patch("cai.download.coordinator.delete_model", new_callable=AsyncMock):
...
async def test_existing_progress_marks_resolved_model_completed() -> None:
    cmd_send, cmd_recv = channel[ForwarderDownloadCommand]()
    event_send, event_recv = channel[Event]()

    coordinator = DownloadCoordinator(
        node_id=NODE_ID,
        shard_downloader=ExistingPendingFakeDownloader(),
        download_command_receiver=cmd_recv,
        event_sender=event_send,
    )

    with patch(
        "cai.download.coordinator.resolve_existing_model",
        return_value=Path("/fake/models/test-model"),
    ):
        task = asyncio.create_task(coordinator._emit_existing_download_progress())
        try:
            completed = await _wait_for_download_completed(event_recv, MODEL_ID)
            assert completed is not None
            assert completed.model_directory == "/fake/models/test-model"
        finally:
            await cmd_send.aclose()
            task.cancel()
            with contextlib.suppress(asyncio.CancelledError):
                await task
