...
from cai.download.download_utils import (
...
class TestResolveExistingModel:
    def test_returns_none_when_no_dirs_have_model(self, tmp_path: Path) -> None:
        ...
        writable.mkdir()
        with (
            patch("cai.download.download_utils.CAI_MODELS_READ_ONLY_DIRS", ()),
            patch("cai.download.download_utils.CAI_MODELS_DIRS", (writable,)),
        ):
    ...
    def test_finds_model_in_read_only_dir(self, tmp_path: Path) -> None:
        ...
        with (
            patch(
                "cai.download.download_utils.CAI_MODELS_READ_ONLY_DIRS", (read_only,)
            ),
            patch("cai.download.download_utils.CAI_MODELS_DIRS", (writable,)),
        ):
    ...
    def test_skips_incomplete_model(self, tmp_path: Path) -> None:
        ...
        with (
            patch(
                "cai.download.download_utils.CAI_MODELS_READ_ONLY_DIRS", (incomplete,)
            ),
            patch("cai.download.download_utils.CAI_MODELS_DIRS", (complete,)),
        ):
            result = resolve_existing_model(MODEL_ID)
    ...
    def test_searches_multiple_read_only_dirs_in_order(self, tmp_path: Path) -> None:
        ...
        writable.mkdir()
        with (
            patch("cai.download.download_utils.CAI_MODELS_READ_ONLY_DIRS", (ro1, ro2)),
            patch("cai.download.download_utils.CAI_MODELS_DIRS", (writable,)),
        ):
            assert resolve_existing_model(MODEL_ID) == ro2 / NORMALIZED

    def test_finds_flat_gguf_in_read_only_dir(self, tmp_path: Path) -> None:
        read_only = tmp_path / "readonly"
        gguf_dir = read_only / "gguf"
        gguf_dir.mkdir(parents=True)
        gguf_path = gguf_dir / "qwen2.5-0.5b-instruct-q4_k_m.gguf"
        ...
        with (
            ...
            patch("cai.download.download_utils.CAI_MODELS_DIRS", ()),
        ):
            assert resolve_existing_model(model_id) == gguf_path

    def test_private_network_alias_finds_public_execution_model_dir(
        self, tmp_path: Path
    ) -> None:
        writable = tmp_path / "writable"
        model_dir = writable / ModelId("Qwen/Qwen3-0.6B-GGUF").normalize()
        model_dir.mkdir(parents=True)
        gguf_path = model_dir / "Qwen3-0.6B-Q8_0.gguf"
        gguf_path.write_bytes(b"gguf")
        ...
        with (
            ...
            assert (
                resolve_existing_model(ModelId("cai-network/Qwen3-0.6B-GGUF"))
                == gguf_path
            )
...
class TestIsReadOnlyModelDir:
    def test_path_under_read_only_dir(self, tmp_path: Path) -> None:
        ...
        with patch("cai.download.download_utils.CAI_MODELS_READ_ONLY_DIRS", (ro,)):
            assert is_read_only_model_dir(ro / NORMALIZED) is True

    def test_path_under_writable_dir(self, tmp_path: Path) -> None:
        writable = tmp_path / "writable"
        with patch("cai.download.download_utils.CAI_MODELS_READ_ONLY_DIRS", ()):
...
class TestSelectDownloadDir:
    def test_picks_first_dir_with_enough_space(self, tmp_path: Path) -> None:
        ...
        with patch("cai.download.download_utils.CAI_MODELS_DIRS", (dir1, dir2)):
    ...
    def test_skips_dir_without_enough_space(self, tmp_path: Path) -> None:
        ...
        with (
            patch("cai.download.download_utils.CAI_MODELS_DIRS", (dir1, dir2)),
    ...
    def test_raises_when_no_dir_has_space(self, tmp_path: Path) -> None:
        ...
        with (
            patch("cai.download.download_utils.CAI_MODELS_DIRS", (dir1,)),
            patch("shutil.disk_usage", side_effect=mock_disk_usage),
    ...
    def test_skips_nonexistent_dir(self, tmp_path: Path) -> None:
        ...
        with (
            patch("cai.download.download_utils.CAI_MODELS_DIRS", (nonexistent,)),
            pytest.raises(InsufficientDiskSpaceError),
        ):
...
class TestDeleteModel:
    ...
    async def dirs(self, tmp_path: Path) -> AsyncIterator[tuple[Path, Path, Path]]:
        ...
        with (
            patch(
                "cai.download.download_utils.CAI_MODELS_DIRS",
                (writable1, writable2, default),
            ),
            patch("cai.download.download_utils.CAI_DEFAULT_MODELS_DIR", default),
        ):
