...
from cai.download.huggingface_utils import (
    filter_repo_objects,
    get_allow_patterns,
    get_auth_headers,
    get_hf_endpoint,
...
from cai.shared.types.worker.shards import ShardMetadata
...
def _model_id_resolution_candidates(model_id: ModelId) -> tuple[ModelId, ...]:
    requested = ModelId(model_id)
    download_repo = private_network_download_repo_id(requested)
    candidates = [requested]
    if download_repo != requested:
        candidates.append(download_repo)
...
def resolve_existing_model(model_id: ModelId) -> Path | None:
    ...
    """
    for candidate_model_id in _model_id_resolution_candidates(model_id):
        custom_path = get_custom_model_local_path(candidate_model_id)
        if custom_path is not None:
            if custom_path.is_file() and custom_path.suffix.lower() == ".gguf":
                return custom_path
            if custom_path.is_dir():
                gguf_candidates = sorted(custom_path.glob("*.gguf"))
                if gguf_candidates:
                    return gguf_candidates[0]
                recursive_candidates = sorted(custom_path.glob("**/*.gguf"))
                if recursive_candidates:
                    return recursive_candidates[0]
                if is_model_directory_complete(custom_path):
                    return custom_path

        normalized = candidate_model_id.normalize()
        for search_dir in (*CAI_MODELS_READ_ONLY_DIRS, *CAI_MODELS_DIRS):
            candidate = search_dir / normalized
            if candidate.is_dir():
                gguf_candidates = sorted(candidate.glob("*.gguf"))
                if gguf_candidates:
                    return gguf_candidates[0]
                recursive_candidates = sorted(candidate.glob("**/*.gguf"))
                if recursive_candidates:
                    return recursive_candidates[0]
                if is_model_directory_complete(candidate):
                    return candidate
            flat_gguf = _resolve_flat_gguf_model(search_dir, candidate_model_id)
            if flat_gguf is not None:
                return flat_gguf
    return None


def _resolve_flat_gguf_model(search_dir: Path, model_id: ModelId) -> Path | None:
    if not search_dir.is_dir():
        return None

    model_key = _compact_model_key(model_id.short())
    if not model_key:
        return None

    matches: list[Path] = []
    for candidate in sorted(search_dir.glob("**/*.gguf")):
        if not candidate.is_file():
            continue
        candidate_key = _compact_model_key(candidate.stem)
        if model_key and model_key in candidate_key:
            matches.append(candidate)

    if not matches:
        return None
    return min(matches, key=lambda path: (len(path.parts), str(path).lower()))


def _compact_model_key(value: str) -> str:
    normalized = value.lower().replace("gguf", "")
    return "".join(ch for ch in normalized if ch.isalnum())


def is_read_only_model_dir(model_dir: Path) -> bool:
    """Check if a model directory lives under a read-only models root."""
    return any(model_dir.is_relative_to(d) for d in CAI_MODELS_READ_ONLY_DIRS)
...
def build_model_path(model_id: ModelId) -> Path:
    ...
    if found is not None:
        return found
    return CAI_DEFAULT_MODELS_DIR / model_id.normalize()
...
def select_download_dir(required_bytes: int) -> Path:
    ...
    """
    for candidate_dir in CAI_MODELS_DIRS:
    ...
    raise InsufficientDiskSpaceError(
        ...
        f"Checked: {[str(d) for d in CAI_MODELS_DIRS]}"
    )
...
async def ensure_cache_dir(model_id: ModelId) -> Path:
    ...
    target = CAI_DEFAULT_MODELS_DIR / "caches" / model_id.normalize()
    await aios.makedirs(target, exist_ok=True)
    return target
...
async def delete_model(model_id: ModelId) -> bool:
    ...
    deleted = False
    for models_dir in CAI_MODELS_DIRS:
    ...
    cache_dir = CAI_DEFAULT_MODELS_DIR / "caches" / normalized
...
async def seed_models(seed_dir: str | Path):
    ...
    await aios.makedirs(CAI_DEFAULT_MODELS_DIR, exist_ok=True)
    dest_dir = CAI_DEFAULT_MODELS_DIR
            ...
            if await aios.path.exists(model_dir):
                ...
                if gguf_candidates:
                    return [
                        FileListEntry(
                            type="file",
                            path=str(gguf_path.relative_to(model_dir)).replace("\\", "/"),
...
async def _fetch_file_list(
    ...
    repo_model_id = private_network_download_repo_id(model_id)
    api_url = f"{get_hf_endpoint()}/api/models/{repo_model_id}/tree/{revision}"
    ...
    async with (
        ...
        if response.status in [401, 403]:
            msg = await _build_auth_error_message(response.status, repo_model_id)
            raise HuggingFaceAuthenticationError(msg)
        elif response.status == 429:
            raise HuggingFaceRateLimitError(
                f"Couldn't download {repo_model_id} because of HuggingFace rate limit."
            )
...
async def file_meta(
    ...
    repo_model_id = private_network_download_repo_id(model_id)
    url = (
        urljoin(f"{get_hf_endpoint()}/{repo_model_id}/resolve/{revision}/", path)
    ...
    async with (
        ...
        if r.status in [401, 403]:
            msg = await _build_auth_error_message(r.status, repo_model_id)
            raise HuggingFaceAuthenticationError(msg)
        content_length = int(
...
async def _download_file(
    ...
    if resume_byte_pos != length:
        repo_model_id = private_network_download_repo_id(model_id)
        url = urljoin(f"{get_hf_endpoint()}/{repo_model_id}/resolve/{revision}/", path)
        headers = await get_download_headers()
        if resume_byte_pos:
...
async def resolve_allow_patterns(shard: ShardMetadata) -> list[str]:
    if shard.model_card.inference_backend == InferenceBackend.LlamaCpp:
        if shard.model_card.preferred_filename:
            return [shard.model_card.preferred_filename]
        return ["*.gguf"]

    if not requires_filtered_downloads(shard):
        return ["*"]
    ...
    try:
        ...
        logger.error(
            f"Error getting shard allow patterns for protected model "
            f"{shard.model_card.model_id=}: {exc}"
        )
        logger.error(traceback.format_exc())
        raise ValueError(
            f"Unable to resolve shard-only download patterns for protected model "
...
async def download_shard(
    ...
    validate_private_network_shard_download(shard, skip_download=skip_download)
    ...
    if not skip_download:
        repo_model_id = private_network_download_repo_id(model_id)
        logger.debug(
            f"Downloading {model_id=} from {repo_model_id=} with {allow_patterns=}"
        )
    ...
    try:
        ...
        return CAI_DEFAULT_MODELS_DIR / model_id.normalize(), not_started_progress
    ...
    if skip_download:
        ...
        target_dir = (
            ...
            else CAI_DEFAULT_MODELS_DIR / model_id.normalize()
        )
    else:
