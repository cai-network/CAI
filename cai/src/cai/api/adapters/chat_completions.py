...
from cai.download.download_utils import create_http_session
...
async def collect_chat_response_payload(
    command_id: CommandId,
    chunk_stream: AsyncGenerator[
        ErrorChunk | ToolCallChunk | TokenChunk | PrefillProgressChunk, None
    ],
) -> ChatCompletionResponse:
    """Collect all token chunks and build a single ChatCompletionResponse."""
    text_parts: list[str] = []
    ...
    async for chunk in chunk_stream:
        logger.debug(
            "collect_chat_response command_id={} chunk_type={} finish_reason={}",
            command_id,
            type(chunk).__name__,
    ...
    if model is None:
        raise ValueError(
            "No output chunks were received from the runner before the response stream ended"
        )
...
async def collect_chat_response(
    ...
    response = await collect_chat_response_payload(command_id, chunk_stream)
