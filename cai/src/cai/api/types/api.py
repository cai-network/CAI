...
from cai.shared.types.text_generation import ReasoningEffort
...
class ModelListModel(BaseModel):
    ...
    inference_backend: str = Field(default="llama_cpp")
    family: str = Field(default="")
    quantization: str = Field(default="")
    base_model: str = Field(default="")
    capabilities: list[str] = Field(default_factory=list)
    gguf_architecture: str = Field(default="")
    shard_compatibility: str = Field(default="")
    layer_range_supported: bool = Field(default=False)
    model_package_manifest_url: str | None = Field(default=None)
    model_package_catalog_id: str | None = Field(default=None)
    model_package_version: str | None = Field(default=None)
    layer_range_probe_abi: str | None = Field(default=None)
    layer_range_probe_report: str | None = Field(default=None)
    layer_range_equivalence_probe_report: str | None = Field(default=None)
    state_format: str | None = Field(default=None)
    activation_state_format: str | None = Field(default=None)
    decode_state_format: str | None = Field(default=None)
    shard_compatibility_reason: str = Field(default="")
...
class AddCustomModelParams(BaseModel):
    ...
    def _validate_source(self) -> "AddCustomModelParams":
        if self.local_path and self.local_path.strip():
            return self
        if self.model_id:
            return self
        raise ValueError("Provide either model_id or local_path")
...
def _normalize_tagged_instance_payload(value: Any) -> Any:
    if isinstance(value, list):
        return [_normalize_tagged_instance_payload(item) for item in value]
    if isinstance(value, dict):
        normalized = {
            key: _normalize_tagged_instance_payload(item) for key, item in value.items()
        }
        if (
            len(normalized) == 1
            and (tag_key := next(iter(normalized.keys()))) in _CREATE_INSTANCE_TAG_KEYS
            and isinstance(normalized[tag_key], dict)
        ):
...
class CreateInstanceParams(BaseModel):
    ...
    @classmethod
    def _normalize_instance_payload(cls, value: Any) -> Any:
        return _normalize_tagged_instance_payload(value)
