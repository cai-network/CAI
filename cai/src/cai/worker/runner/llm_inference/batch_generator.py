...
from cai.shared.constants import CAI_MAX_CONCURRENT_REQUESTS
from cai.shared.types.chunks import ErrorChunk, PrefillProgressChunk
from cai.shared.types.common import ModelId
from cai.shared.types.events import ChunkGenerated, Event
from cai.shared.types.mlx import Model
from cai.shared.types.tasks import CANCEL_ALL_TASKS, TaskId, TextGeneration
from cai.shared.types.text_generation import TextGenerationTaskParams
from cai.shared.types.worker.runner_response import GenerationResponse, ToolCallResponse
from cai.utils.channels import MpReceiver, MpSender
from cai.worker.engines.mlx.cache import KVPrefixCache
from cai.worker.engines.mlx.generator.batch_generate import CAIBatchGenerator
from cai.worker.engines.mlx.generator.generate import (
    PrefillCancelled,
    mlx_generate,
    warmup_inference,
)
from cai.worker.engines.mlx.utils_mlx import (
    apply_chat_template,
    mx_all_gather_tasks,
    mx_any,
)
from cai.worker.engines.mlx.vision import VisionProcessor
...
CAI_RUNNER_MUST_FAIL = "cai RUNNER MUST FAIL"
CAI_RUNNER_MUST_OOM = "cai RUNNER MUST OOM"
CAI_RUNNER_MUST_TIMEOUT = "cai RUNNER MUST TIMEOUT"


def _check_for_debug_prompts(task_params: TextGenerationTaskParams) -> None:
    """Check for debug prompt triggers in the input."""
    from cai.worker.engines.mlx.utils_mlx import mlx_force_oom
    ...
    if not prompt:
        return
    if CAI_RUNNER_MUST_FAIL in prompt:
        raise Exception("Artificial runner exception - for testing purposes only.")
    if CAI_RUNNER_MUST_OOM in prompt:
        mlx_force_oom()
    if CAI_RUNNER_MUST_TIMEOUT in prompt:
        time.sleep(100)
...
class BatchGenerator(InferenceGenerator):
    ...
    _all_tasks: dict[TaskId, TextGeneration] = field(default_factory=dict, init=False)
    _queue: deque[TextGeneration] = field(default_factory=deque, init=False)
    _mlx_gen: CAIBatchGenerator = field(init=False)
    _active_tasks: dict[
        int,
        tuple[
            TextGeneration,
    ...
    def __post_init__(self) -> None:
        self._mlx_gen = CAIBatchGenerator(
            model=self.model,
            tokenizer=self.tokenizer,
            group=self.group,
            kv_prefix_cache=self.kv_prefix_cache,
    ...
    def step(
        ...
        while self._queue and len(self._active_tasks) < CAI_MAX_CONCURRENT_REQUESTS:
            task = self._queue.popleft()
            try:
