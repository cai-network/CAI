...
from cai.shared.types.common import CommandId, ModelId
...
from cai.shared.types.worker.runner_response import (
    GenerationResponse,
    ToolCallResponse,
)
...
from cai.worker.runner.llm_inference.batch_generator import (
    BatchGenerator,
    InferenceGenerator,
    SequentialGenerator,
)
...
class Runner:
    ...
    def send_response(
        ...
        logger.debug(
            "runner send_response command_id={} rank={} type={} finish_reason={}",
            command_id,
            self.device_rank,
...
class Builder:
    ...
    def build(
        ...
        distributed_prefix_cache_enabled = True
        if self.group is not None and self.group.size() > 1:
            distributed_prefix_cache_enabled = os.environ.get(
                "CAI_ENABLE_DISTRIBUTED_PREFIX_CACHE", ""
            ).lower() in ("1", "true", "yes", "on")
            if not distributed_prefix_cache_enabled:
                logger.info(
                    "disabling distributed KV prefix cache for multi-node runner"
                )

        kv_prefix_cache = (
            KVPrefixCache(self.group) if distributed_prefix_cache_enabled else None
        )

        device_rank = 0 if self.group is None else self.group.rank()
        if os.environ.get("CAI_NO_BATCH"):
