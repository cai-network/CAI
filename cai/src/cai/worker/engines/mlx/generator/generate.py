...
from cai.shared.types.mlx import KVCacheType, Model
...
from cai.worker.engines.mlx.auto_parallel import (
    PipelineFirstLayer,
    PipelineLastLayer,
    clear_prefill_sends,
    flush_prefill_sends,
...
from cai.worker.engines.mlx.cache import (
    CacheSnapshot,
    KVPrefixCache,
    encode_prompt,
    has_non_kv_caches,
...
from cai.worker.engines.mlx.constants import (
...
from cai.worker.engines.mlx.vision import (
    MediaRegion,
    VisionProcessor,
    VisionResult,
    get_inner_model,
...
def warmup_inference(
    ...
    max_output_tokens = _warmup_max_output_tokens()
    if max_output_tokens <= 0:
        logger.info(
            "skipping inference warmup because CAI_WARMUP_MAX_OUTPUT_TOKENS <= 0"
        )
        mx_barrier(group)
        return 1
...
def _warmup_max_output_tokens() -> int:
    raw_value = os.getenv("CAI_WARMUP_MAX_OUTPUT_TOKENS", "50").strip()
    try:
        return max(0, int(raw_value))
    except ValueError:
        logger.warning(
            "invalid CAI_WARMUP_MAX_OUTPUT_TOKENS={!r}; falling back to 50",
            raw_value,
        )
        return 50
