...
class TestAllocateLayersProportionally:
    ...
    def test_custom_minimum_rebalances_from_largest_node(self):
        result = allocate_layers_proportionally(
            total_layers=20,
            memory_fractions=[0.95, 0.025, 0.025],
            minimum_layers_per_node=2,
        )
        assert result == [16, 2, 2]
        assert sum(result) == 20

    def test_custom_minimum_rejects_too_few_layers(self):
        with pytest.raises(ValueError, match="need at least 2 layers per node"):
            allocate_layers_proportionally(
                total_layers=3,
