...
ENTRYPOINT = SOURCE_ROOT / "cai" / "__main__.py"
DASHBOARD_DIR = PROJECT_ROOT / "dashboard" / "build"
RESOURCES_DIR = PROJECT_ROOT / "resources"
CAI_SHARED_MODELS_DIR = SOURCE_ROOT / "cai" / "shared" / "models"

if not ENTRYPOINT.is_file():
    raise SystemExit(f"Unable to locate cai entrypoint: {ENTRYPOINT}")
...
if not CAI_SHARED_MODELS_DIR.is_dir():
    raise SystemExit(f"Shared model assets are missing: {CAI_SHARED_MODELS_DIR}")
...
DATAS: list[tuple[str, str]] = [
    ...
    (str(CAI_SHARED_MODELS_DIR), "CAI/shared/models"),
]
...
if MACMON_PATH is None:
    raise SystemExit(
        ...
        "Install the pinned fork used by cai via: "
